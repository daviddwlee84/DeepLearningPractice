...
def train_test_experiment(train_set, test_set, encoder: Encoding):
    ...
    CRF_train_test_model = CRF(num_words, num_features, num_tags,
                               model_dir='model/ner_train_test', model_name='one-hot_crf')
...
def train_all_prediction(all_set, final_x, final_seq_len, encoder: Encoding):
    ...
    CRF_all_model = CRF(num_words, num_features, num_tags,
                        model_dir='model/ner_all', model_name='one-hot_crf')
...
if __name__ == "__main__":
    ...
    test30percent = from_ner_numpy_to_evaluable_format(
        test_x, test_predict, test_seq_len, word_to_id, NER_LabelEncode, 'ner_test30percent.txt')
    # test from output file
    # test30percent = get_ner_labels_from_file(
    #     'ner_test30percent.txt', use_utf16_encoding=False)
